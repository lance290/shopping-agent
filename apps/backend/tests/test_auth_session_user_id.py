...
async def test_auth_session_has_user_id(client: AsyncClient, session):
    ...
    login_code = AuthLoginCode(email=None, phone_number=phone, code_hash=code_hash)
    session.add(login_code)
    await session.commit()
    
    # 2. Call auth/verify
    response = await client.post("/auth/verify", json={"phone": phone, "code": code})
    ...
    assert auth_session.phone_number == phone
