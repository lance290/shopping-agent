...
async def test_mint_session_endpoint(client: AsyncClient, monkeypatch):
    ...
    resp = await client.post("/test/mint-session", json={"email": "test@example.com"})
    assert resp.status_code == 404
    
    # 2. Test when E2E_TEST_MODE is set (should be 200)
    monkeypatch.setenv("E2E_TEST_MODE", "1")
    email = "mint_test@example.com"
    resp = await client.post("/test/mint-session", json={"email": email})
