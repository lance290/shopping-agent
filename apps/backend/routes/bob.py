...
router = APIRouter(prefix="/bob", tags=["bob-chatbot"])
...
async def resend_inbound(
    ...
    background_tasks.add_task(process_bob_message, sender, body, session, "email", None)
...
async def twilio_inbound(
    ...
    if user and user.email:
        background_tasks.add_task(
            process_bob_message, user.email, body, session, "sms", sender_phone,
        )
    else:
        logger.info(f"[Bob] Unknown phone {sender_phone}. Sending onboarding SMS.")
        send_bob_onboarding_sms(sender_phone)
